...
namespace osu.Framework.Tests.Visual.Layout {
	[TestFixture]
	public class TestSceneNoWrapVertical : FlexboxTestScene {
		...
		public TestSceneNoWrapVertical () {
			Flexbox flexbox = new() {
				Width = 100,
				Height = 800,
			...
			for ( int i = 0; i < itemCount; i++ ) {
				...
				AddSliderStep( "Max width", 0, 1000, 100, v => item.MaxWidth = v.Pixels() );
				AddSliderStep( "Min height", 0, 1000, 0, v => item.MinHeight = v.Pixels() );
				AddSliderStep( "Max height", 0, 1000, 1000, v => item.MaxHeight = v.Pixels() );
			}
		}
	}
}
